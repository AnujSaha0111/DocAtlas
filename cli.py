...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="AI Provider to use (openai, gemini, or groq)"
    )
    ...
    parser.add_argument(
        ...
        help="Model to use (e.g., gpt-3.5-turbo, gpt-4, gemini-1.5-flash, llama3-8b-8192)"
    )
    ...
    parser.add_argument(
        "--api-key", 
        type=str,
        help="API key for the selected provider (if not set in environment variable)"
    )
...
def main():
    ...
    if args.provider == "openai":
        ...
        if not api_key:
            logging.error("Gemini API key not found. Please set GEMINI_API_KEY environment variable or use --api-key.")
            sys.exit(1)
            
    elif args.provider == "groq":
        api_key = api_key or os.getenv("GROQ_API_KEY")
        if not api_key:
            logging.error("Groq API key not found. Please set GROQ_API_KEY environment variable or use --api-key.")
            sys.exit(1)
    ...
    try:
        ...
        extractor = ModuleExtractor(api_key=api_key, model=args.model, provider=args.provider)
        
        logging.info(f"Crawling with max depth of {max_depth} (main page + direct links only)")
        logging.info(f"Using {args.provider} model: {args.model}")
